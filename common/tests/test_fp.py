...
Verifies:
    ...
    7. Metrics (spacetime volume, idle slots, no-fault probability)
"""
...
def test_fp_2d_default_uses_folded_gamma():
    """The public builder defaults to the current shallow construction."""
    L = 3
    perm = structured_permutations(L)["transpose"]
    assert build_fp_2d(L, perm).gamma_method == GammaMethod.FOLDED.value
...
def test_end_to_end_all_baselines(L):
    ...
    for perm_name, perm in perms:
        ...
        # Folded admissible Gamma-coset sibling
        res_folded = build_fp_2d(L, perm, GammaMethod.FOLDED)
        circ_folded_full = (
            res_folded.circuit + cirq.Circuit([cirq.I(q) for q in canonical_qubits])
        )
        result_folded = sim.simulate(
            circ_folded_full,
            qubit_order=canonical_qubits,
            initial_state=init_state.copy(),
        )
        dm_folded = np.outer(
            result_folded.final_state_vector,
            np.conj(result_folded.final_state_vector),
        )
...
def test_no_fault_probability_formula_and_legacy_wrapper():
    """The named no-fault estimate follows its independent-location model."""
    probability = no_fault_probability(
        100, 200, p_2q=0.001, p_idle=0.0001
    )
    expected = (1.0 - 0.001) ** 100 * (1.0 - 0.0001) ** 200
    assert abs(probability - expected) < 1e-12
    assert multiplicative_fidelity(
        100, 200, p_2q=0.001, p_idle=0.0001
    ) == probability
