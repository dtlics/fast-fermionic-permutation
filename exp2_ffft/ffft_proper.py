...
build_ffft_core(L)
    The minimal Cooley-Tukey arithmetic kernel.  It assumes raster-labelled
    input and leaves column-major-raster output (see below).
...
------------------------------
Assumes logical modes are already in **row-major raster** layout.  The qubit
Jordan--Wigner order remains the physical row snake; "raster" describes
which logical mode occupies each qubit.  Starting from the canonical snake
mode layout requires the odd-row reversal used by `build_ffft_proper`.
...
This is a *known, fixed* output relabeling that depends only on L.  Downstream
code that operates in the DFT-output basis can use this mapping directly
without appending the folded-Gamma FP transpose, saving ``10L + O(1)``
CNOT-equivalent entangling depth.

Example for L = 3 (N = 9):

    Input (row-major raster):     Output (col-major raster):
      0  1  2                       0  3  6
      3  4  5                       1  4  7
      6  7  8                       2  5  8
...
--------------------------------
Wraps the core with:
  - Odd-row reversal before  (snake -> raster, depth ``2L``)
  - Folded-Gamma FP transpose after (column-major raster -> row-major snake,
    depth at most ``10L + 8`` for even L and ``10L + 12`` for odd L when
    ``L >= 7``; the explicit small-size fallback is used below that threshold)
...
def _build_gamma(L, sq, gamma_method):
    ...
    ``FOLDED`` is the shallowest universal option and is valid for the full
    number-conserving column FFFT, not only for permutation circuits.
    """
    if gamma_method == GammaMethod.ANCILLA:
        ...
        circ, _ = build_gamma_folded(L, sq=sq)
        return circ, []
    else:
...
def build_ffft_core(
    ...
    Assumes logical input modes are in row-major raster layout.  After the
    circuit, grid position (r, c) holds DFT output mode k = r + c*L
    (column-major raster).  See the module docstring for the full mapping.

    For the canonical row-snake mode layout, prepend
    :func:`_build_odd_row_reversal` or use :func:`build_ffft_proper`.
    ...
    Args:
        ...
        gamma_method: which universal Gamma construction to use.  ``FOLDED``
            has CNOT-equivalent two-qubit depth ``2L+O(1)`` and is full-Fock verified.
