...
SYK_GENERATOR_MODEL = "sparse_syk_majorana_v1"
QUARTET_SAMPLING_MODEL = "enumerate_lt_2m_else_binomial_rejection_v1"
COUPLING_MODEL = "gaussian_std_sqrt_6_over_M_cubed_v1"
PARENT_COLORING_MODEL = "networkx_greedy_largest_first_parent_v1"
...
def _sha256_json(value) -> str:
    """Hash a nested integer structure using a stable compact JSON encoding."""
...
def instance_reproducibility_provenance(
    instance: SYKInstance,
    rng_seed: int,
) -> Dict[str, object]:
    """Return exact generator and structural identity for a saved instance.

    Couplings are intentionally excluded from the structural fingerprints:
    their normalization changes rotation angles but not quartets, coloring,
    packing permutations, circuit shape, or any reported resource count.
    ``COUPLING_MODEL`` records the chosen normalization separately.
    """
    return {
        "rng_seed": int(rng_seed),
        "syk_generator_model": SYK_GENERATOR_MODEL,
        "quartet_sampling_model": QUARTET_SAMPLING_MODEL,
        "coupling_model": COUPLING_MODEL,
        "parent_coloring_model": PARENT_COLORING_MODEL,
        "numpy_version": np.__version__,
        "networkx_version": nx.__version__,
        "quartets_sha256": _sha256_json(instance.quartets),
        "coloring_sha256": _sha256_json({
            "color_groups": instance.color_groups,
            "packing_perms": instance.packing_perms,
        }),
    }
...
def generate_sparse_syk(
    ...
    # A fixed Gaussian scale is sufficient for this routing-only benchmark;
    # circuit structure and every reported resource are independent of it.
    couplings = rng.normal(0, np.sqrt(6.0 / M**3), size=len(quartets))
...
def _sample_n_distinct(
    ...
    if len(quartets) != n:
        raise RuntimeError(
            f"sampled only {len(quartets)} of {n} distinct quartets "
